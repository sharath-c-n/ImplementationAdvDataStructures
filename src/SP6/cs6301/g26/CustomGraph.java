...
public class CustomGraph implements Iterable<CustomGraph.Vertex> {
    ...
    public static CustomGraph readDirectedGraph(Scanner in) {
        return readGraph(in, true);
    }

    // read an undirected graph using the Scanner interface
    public static CustomGraph readGraph(Scanner in) {
        return readGraph(in, false);
    }

    public static CustomGraph readGraph(Scanner in, boolean directed) {
        // read the graph related parameters
        int n = in.nextInt(); // number of vertices in the graph
        int m = in.nextInt(); // number of edges in the graph

        // create a graph instance
        CustomGraph g = new CustomGraph(n);
        g.directed = directed;
        for (int i = 0; i < m; i++) {
            int u = in.nextInt();
            int v = in.nextInt();
